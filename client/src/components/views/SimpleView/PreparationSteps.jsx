  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      {Object.entries(recipe.subRecipes || {}).map(([subRecipeId, subRecipe], index) => {
        ...
        return (
          <Box key={subRecipeId} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Box sx={{ 
                      ...
                      {(step.time || step.stepType || step.stepMode) && (
                        <Box sx={{ 
                          ...
                          {step.stepMode === 'passive' && (step.time || step.stepType) && (
                            <Typography 
                              variant="body2"
                              sx={{ 
                                color: 'text.disabled',
                          ...
                          {step.time && (step.stepMode === 'passive' || parseTimeToMinutes(step.time) > 10) && (
                            <Typography
                              variant="body2"
                              sx={{ 
                                color: 'text.disabled',
                            ...
                            >
                              <TimeDisplay timeString={step.time} detailed={true} />
                            </Typography>
                          )}
                          {step.time && step.stepType && (step.stepMode === 'passive' || parseTimeToMinutes(step.time) > 10) && (
                            <Typography 
                              variant="body2"
                              sx={{ 
                                color: 'text.disabled',
                          ...
                          {step.stepType && (
                            <>
                              {step.stepType === 'prep' && (
                                <RestaurantMenuOutlinedIcon sx={{ 
                                  fontSize: '1.2rem',
                                  color: 'text.disabled'
                                }} />
                              )}
                              {step.stepType === 'combine' && (
                                <BlenderOutlinedIcon sx={{ 
                                  fontSize: '1.2rem',
                                  color: 'text.disabled'
                                }} />
                              )}
                              {step.stepType === 'cook' && (
                                <LocalFireDepartmentOutlinedIcon sx={{ 
                                  fontSize: '1.2rem',
                                  color: isCompleted ? 'text.disabled' : 'warning.main'
                                }} />
                              )}
                            </>
                          )}
