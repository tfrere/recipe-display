"""
Unified V2 Prompt — Pass 2 of the 2-pass pipeline (DAG construction).

This prompt receives PREFORMATTED structured text from Pass 1 and builds
the complete RecipeV2 JSON graph (DAG). It no longer needs to parse raw
...
INGREDIENTS:
- 4 cuisses de poulet [chicken thighs] {poultry}
- 250g champignons de Paris [mushrooms] {produce}, émincés [sliced]
- 1 oignon [onion] {produce}, ciselé [diced]
- 30g beurre [butter] {dairy}
- 200ml crème fraîche épaisse [heavy cream] {dairy}
- 100ml vin blanc sec [dry white wine] {beverage}
- 200ml bouillon de volaille [chicken broth] {pantry}
- 1 cs farine [flour] {pantry}
- 2 branches thym frais [fresh thyme] {spice}
...
FEW_SHOT_EXPECTED_OUTPUT = {
    ...
    "ingredients": [
        ...
        {"id": "flour", "name": "farine", "name_en": "flour", "quantity": 1, "unit": "tbsp", "category": "pantry", "preparation": None, "notes": None, "optional": False},
...
## 7. Data formatting
- ISO 8601 durations: "5min" → "PT5M", "1h30min" → "PT1H30M", "2h" → "PT2H"
...
1. If this is NOT a valid recipe (login page, error, empty content), respond with:
